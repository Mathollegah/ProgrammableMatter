...
def build_new_configuration(config, state):
    if config.load_file == "" and not config.random_configuration:
        state.dodecahedrons = [Dodecahedron(0,0,0)]
        i=0
        while i < config.number_of_tiles-1:
            x = random.choice(state.dodecahedrons)
            ...
            if d[1] == None:
                ...
                tmp.find_neighbours(state.dodecahedrons)
    ...
    elif config.load_file == "" and config.random_configuration:
        build_random_configuration(config, state)

    else:
        f = open(config.load_file, "r")
        txt = f.read()
        f.close()
        nodes = txt.split()
        ...
        for i in state.dodecahedrons:
            if (i.x,i.y,i.z) == (tmp_robot_pos[0], tmp_robot_pos[1], tmp_robot_pos[2]):
                state.global_start_node = i
                break



def build_random_configuration(config, state):
    ...
    max_component = 0
    comp_name = None
    while max_component < config.number_of_tiles:
        x,y,z = pick_position(boxsize)

        exists = False
        for tile in state.dodecahedrons:
        ...
        if not exists:
            ...
            for comp in tmp.component:
                for dod in state.dodecahedrons:
    ...
    state.dodecahedrons = new_dodecas

    dodeca = random.choice(state.dodecahedrons)
    tmp_x = dodeca.x
    tmp_y = dodeca.y
    tmp_z = dodeca.z
    for i in state.dodecahedrons:
...
build_new_configuration(config, state)

if not config.trainmodel:
    app = Simulator3D(ConstRobot(state), config, state)

    if not config.onlygenerate or config.run_silent:
        print("Count: " + str(state.global_move_count) + ", Potential_X: " + str(
            pot.potential_x(app.grabbed_tile)) + ", " + "Potential_Z: " + str(
            pot.potential_z(app.grabbed_tile)))

        #app.run()
        while not config.visualize:
            app.moveRobotNoAnimation()

            if app.robot.state == 'terminate':
                print("Count: " + str(state.global_move_count) + ", Potential_X: " + str(pot.potential_x(app.grabbed_tile)) + ", " + "Potential_Z: " + str(pot.potential_z(app.grabbed_tile)))
                break
        ...
        print("Number of Dodecahedrons: ", len(state.dodecahedrons))
else:
    game = Game()